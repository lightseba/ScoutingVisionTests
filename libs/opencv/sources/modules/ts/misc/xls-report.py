...
"""
    ...
    * 'configuration_matchers': [{'properties': {string: object}, 'name': string}]
        Instructions for matching com.testcode.test run property sets to configuration names.
...
class Collector(object):
    ...
    def collect_from(self, xml_path, default_configuration):
        ...
        for com.testcode.test in run.tests:
            test_results = module_tests.setdefault((com.testcode.test.shortName(), com.testcode.test.param()), {})
            new_result = com.testcode.test.get("gmean") if com.testcode.test.status == 'run' else com.testcode.test.status
...
def main():
    ...
    arg_parser.add_argument('sheet_dirs', nargs='+', metavar='DIR', help='directory containing perf com.testcode.test logs')
    ...
    for sheet_path in args.sheet_dirs:
        ...
        for module, tests in sorted(collector.tests.iteritems()):
            for ((com.testcode.test, param), configs) in sorted(tests.iteritems()):
                sheet.write(row, 0, module, module_styles.get(module, xlwt.Style.default_style))
                sheet.write(row, 1, com.testcode.test)
