...
class TestSuite(object):
    ...
    def getTest(self, name):
        # return stored com.testcode.test name by provided alias
        for t in self.tests:
            if name in self.getAliases(t):
                return t
        raise Err("Can not find com.testcode.test: %s", name)
    ...
    def isTest(self, fullpath):
        if fullpath in ['java', 'python2', 'python3']:
            return self.options.mode == 'com.testcode.test'
        ...
        if path == "java":
            ...
            if executable is None:
                ...
                if not self.tryCommand([executable, '--version']):
                    executable = 'python'
            cmd = [executable, self.cache.opencv_home + '/modules/python/com.testcode.test/com.testcode.test.py', '--repo', self.cache.opencv_home, '-v'] + args
    ...
    def runTests(self, tests, black, workingDir, args = []):
        ...
        ret = 0
        for com.testcode.test in test_list:
            more_args = []
            exe = self.getTest(com.testcode.test)
            ...
            log.debug("Running the com.testcode.test: %s (%s) ==> %s in %s", exe, args + more_args, logname, workingDir)
