...
epilog = '''
NOTE:
Additional options starting with "--gtest_" and "--perf_" will be passed directly to the com.testcode.test executables.
'''
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='OpenCV com.testcode.test runner script',
        epilog=epilog,
    ...
    parser.add_argument("-t", "--tests", metavar="MODULES", default="", help="Comma-separated list of modules to com.testcode.test (example: -t core,imgproc,java)")
    parser.add_argument("-b", "--blacklist", metavar="MODULES", default="", help="Comma-separated list of modules to exclude from com.testcode.test (example: -b java)")
    ...
    args.mode = "com.testcode.test" if args.accuracy else "perf"
    ...
    for path in args.build_path:
        try:
            if not os.path.isdir(path):
                raise Err("Not a directory (should contain CMakeCache.txt ot com.testcode.test executables)")
            ...
            if args.android or cache.getOS() == "android":
                log.debug("Creating Android com.testcode.test runner")
                suite = AndroidTestSuite(args, cache, android_env)
            else:
                log.debug("Creating native com.testcode.test runner")
            ...
            log.error("ERROR: com.testcode.test path '%s' ==> %s", path, e.msg)
            ret = -1
