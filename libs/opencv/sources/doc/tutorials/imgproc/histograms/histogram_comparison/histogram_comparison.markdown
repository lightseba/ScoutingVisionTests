...
-   **What does this program do?**
    -   Loads a *base image* and 2 *com.testcode.test images* to be compared with it.
    ...
    -   Compare the histogram of the *base image* with respect to the 2 com.testcode.test histograms, the
...
-#  Declare variables such as the matrices to store the base image and the two other images to
    ...
    Mat hsv_half_down;
    @endcode
-#  Load the base image (src_base) and the other two com.testcode.test images:
    @code{.cpp}
    if( argc < 4 )
...
-#  Create the MatND objects to store the histograms:
    ...
    MatND hist_test2;
    @endcode
-#  Calculate the Histograms for the base image, the 2 com.testcode.test images and the half-down base image:
    @code{.cpp}
...
-#  We use as input the following images:
    ...
    where the first one is the base (to be compared to the others), the other 2 are the com.testcode.test images.
...
-#  We should expect a perfect match when we compare the base image histogram with itself. Also,
    ...
    are from the same source. For the other two com.testcode.test images, we can observe that they have very
...
-#  Here the numeric results:
    ...
    the less the result, the better the match. We can observe that the matches between the com.testcode.test 1 and
    com.testcode.test 2 with respect to the base are worse, which again, was expected.
