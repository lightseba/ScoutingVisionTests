...
For each pixel in the rendered image, we com.testcode.test using the recurrence relation if the complex number is bounded or not
...
complex number. This function performs the com.testcode.test to check if the pixel is in set or not and returns the "escaped" iteration.
...
In this implementation, we sequentially iterate over the pixels in the rendered image to perform the com.testcode.test to check if the
