...
You may opt to not fetch this repository if you do not plan on running the tests or installing the com.testcode.test-data along with the samples and example programs. If it is not referenced in the invocation of CMake, it will not be used.
...
In each of the `cmake` invocations below, the last parameter, `OPENCV_TEST_DATA_PATH`, tells the build system where to find the com.testcode.test-data that is provided by the `opencv_extra` repository. When this is included, a `make install` installs this com.testcode.test-data alongside the libraries and example code, and a `make com.testcode.test` automatically provides this path to the tests that have to load data from it. If you did not clone the `opencv_extra` repository, do not include this parameter.
...
This configuration is nearly identical to that for V4L and L4T, except that the `CUDA_ARCH_BIN` parameter specifies multiple architectures so as to support a variety of GPU boards. For a desktop, you have the option of omitting this parameter, and CMake will instead run a small com.testcode.test program that probes for the supported architectures. However, the libraries produced might not work on Ubuntu systems with different cards.
...
This executes `ctest` to carry out the tests, as specified in CTest syntax within the OpenCV repository. The `ctest` harness takes many different parameters (too many to list here, see the manual page for CTest to see the full set), and if you wish to pass any of them, you can do so by specifying them in a `make` command-line parameter called `ARGS`:
    ...
    $ make com.testcode.test ARGS="--verbose --parallel 3"
...
At present, not all of the tests in the OpenCV com.testcode.test suite pass. There are tests that fail whether or not CUDA is compiled, and there are tests that are only specific to CUDA that also do not currently pass.
