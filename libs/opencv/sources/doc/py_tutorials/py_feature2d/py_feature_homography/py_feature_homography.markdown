...
First, as usual, let's find SIFT features in images and apply the ratio com.testcode.test to find the best
matches.
@code{.py}
import numpy as np
import cv2 as cv
...
# store all the good matches as per Lowe's ratio com.testcode.test.
good = []
for m,n in matches:
