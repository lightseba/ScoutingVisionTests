...
com.testcode.test = x[:,50:100].reshape(-1,400).astype(np.float32) # Size = (2500,400)

# Create labels for train and com.testcode.test data
k = np.arange(10)
train_labels = np.repeat(k,250)[:,np.newaxis]
test_labels = train_labels.copy()

# Initiate kNN, train the data, then com.testcode.test it with com.testcode.test data for k=1
knn = cv.ml.KNearest_create()
knn.train(train, cv.ml.ROW_SAMPLE, train_labels)
ret,result,neighbours,dist = knn.findNearest(com.testcode.test,k=5)
...
10000 as com.testcode.test samples. We should change the alphabets to ascii characters because we can't work with
alphabets directly.
@code{.py}
import cv2 as cv
import numpy as np
...
# split the data to two, 10000 each for train and com.testcode.test
train, com.testcode.test = np.vsplit(data,2)

# split trainData and testData to features and responses
responses, trainData = np.hsplit(train,[1])
labels, testData = np.hsplit(com.testcode.test,[1])
